...
def summarize_text(
    ...
    cfg.chain_logger.put("reading", f"å…± {text_length} å­—éœ€è¦é˜…è¯»")
    ...
    if cfg.fast_llm_model in ["llama"] and len(chunks) > 1:
        ...
        for i in range(len(chunks) // batch_size + 1):
            ...
            cnt += len(batch)
            cfg.chain_logger.put("reading", f"{cnt} / {len(chunks)} ä¸ªæ®µè½")
    else:
        for i, chunk in enumerate(chunks):
            ...
            cfg.chain_logger.put("reading", f"ç¬¬ {i + 1} / {len(chunks)} ä¸ªæ®µè½")
    ...
    cfg.chain_logger.put("reading", f"æ€»ç»“è¿™ {len(chunks)} ä¸ªæ®µè½")
...
def create_message(chunk: str, question: str) -> Dict[str, str]:
    ...
    """
    return f'"""{chunk}""" åŸºäºŽä¸Šè¿°æ–‡æœ¬å›žç­”ä¸‹é¢çš„é—®é¢˜ ' +\
        f'é—®é¢˜: "{question}" -- å‡å¦‚æ— æ³•å›žç­”è¿™ä¸ªé—®é¢˜ï¼Œåˆ™æ€»ç»“ä¸Šè¿°æ–‡æœ¬ï¼š'


if __name__ == "__main__":
    # Example usage
    text_en = "This is an example sentence. Here's another one! And a third?"
    text_zh = "è¿™æ˜¯ä¸€ä¸ªç¤ºä¾‹å¥å­ã€‚è¿™æ˜¯å¦ä¸€ä¸ªï¼è¿˜æœ‰ç¬¬ä¸‰ä¸ªï¼Ÿ"
